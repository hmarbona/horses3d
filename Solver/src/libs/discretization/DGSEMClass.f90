...
Module DGSEMClass
   ...
   abstract interface
      SUBROUTINE ComputeTimeDerivative_f( mesh, particles, time, mode, HO_Elements, Level)
         use SMConstants
         use HexMeshClass
         use ParticlesClass
         IMPLICIT NONE
...
!  -------------------------------------------------------------------
   subroutine DetermineCFL(self, deltat, percentile, cfl_percentile, globalMax, globalMin, globalMaxCFLInterf)
      use VariableConversion
      use MPI_Process_Info
      implicit none
      !------------------------------------------------
	  ...
	  real(kind=RP),intent(in)   :: percentile
	  real(kind=RP),intent(out)  :: cfl_percentile
      ...
      real(kind=RP)                 :: cfl, sum_cfl_percentile          ! cfl - Advective
...
!$omp parallel shared(self,elementCFL,NodalStorage,flowIsNavierStokes, deltat, percentile, maxCFLInterfaceID) default(private)
...
!$omp end parallel
	  ...
	  cfl_percentile     = elementCFL(FLOOR(SIZE(self % mesh % elements)*percentile))
      ...
      if ( MPI_Process % doMPIAction ) then
		 ...
		 call mpi_allreduce(minCFL, globalMin, 1, MPI_DOUBLE, MPI_MIN, &
                            MPI_COMM_WORLD, ierr)
         call mpi_allreduce(cfl_percentile, sum_cfl_percentile, 1, MPI_DOUBLE, MPI_SUM, &
                            MPI_COMM_WORLD, ierr)
		 call MPI_Comm_size(MPI_COMM_WORLD, nProcs, ierr)
		 cfl_percentile = sum_cfl_percentile/nProcs
      end if
#endif
