        ...
        do spongeEID = 1, self % nElements
            ...
            associate(e => mesh % elements(eID))
                do k = 0, e % Nxyz(3) ; do j = 0, e % Nxyz(2) ; do i = 0, e % Nxyz(1)
                    ...
                    e % storage % S_NS(1:5,i,j,k) = e % storage % S_NS(1:5,i,j,k) - e % storage % intensitySponge(i,j,k) * (e % storage % Q(1:5,i,j,k) - e % storage % QbaseSponge(1:5,i,j,k))
					!e % storage % Q(1,i,j,k) = e % storage % Q(1,i,j,k)  +   (e % storage % QbaseSponge(1,i,j,k) - e % storage % Q(1,i,j,k)) * (e % storage % intensitySponge(i,j,k)/self % amplitude)
                end do         ; end do          ; end do
            end associate
        end do
!$omp end do
